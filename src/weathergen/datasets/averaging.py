...
class AveragingReader(DataReaderTimestep):
    def __init__(self, wrapped_reader: DataReaderTimestep):
        ...
        self.averaging_geoinfo_idx = [
            idx for idx, channel in enumerate(self.geoinfo_channels)
        ...
        self.non_averaging_geoinfo_idx = [idx for idx, _ in enumerate(self.geoinfo_idx) if idx not in self.averaging_geoinfo_idx]
    ...
    def _get(self, idx: TIndex, channels_idx: list[int]) -> ReaderData:
        ...
        data = pd.DataFrame(
            np.concat(
                [
                    rdata.coords,
                    rdata.geoinfos[:, self.averaging_geoinfo_idx],
                    rdata.data
                ], axis=1
            ),
