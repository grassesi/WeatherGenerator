...
"""Tests for the Coupler driver: chunk interleaving and the rollout derivation.

Alignment is no longer checked here: components are kept on one time axis by refusing to
substitute a window (test_config.strict_batches, enforced in MultiStreamDataSampler), rather
than by comparing their valid times after the fact.
...
def make_rollout(
    chunk_length="24:00:00", num_chunks=4, num_samples=2, forecast_offset=1, **extra
):
    """extra carries the optional rollout keys, so omitting one exercises its default."""
...
def test_num_workers_is_zero_unless_the_rollout_asks_for_more():
    """Each component forks its own worker pool, so the default keeps loading in-process."""
    components = atmo_ocean_cfs()
    derive(components, ATMO_OCEAN)
    for cf in components.values():
        assert cf.data_loading.num_workers == 0

    components = atmo_ocean_cfs()
    derive(components, ATMO_OCEAN, make_rollout(num_workers=12))
    for cf in components.values():
        assert cf.data_loading.num_workers == 12


def test_a_negative_worker_count_is_rejected():
    with pytest.raises(ValueError, match="num_workers"):
        make_rollout(num_workers=-1)
...
def test_component_options_never_fall_back_to_sys_argv(monkeypatch):
    ...
    checkpoint = ModelCheckpoint("Atmo", "abc@0")
